...
static USBPD_StatusTypeDef  PPM_SetUSB_OperationRole(uint8_t PortNumber, UCSI_USB_OPERATION_ROLE_t Role)
{
  ...
  if (DPM_Params[PortNumber].PE_IsConnected)
  {
...
static USBPD_StatusTypeDef  PPM_GetPDOs(uint8_t PortNumber, PUCSI_GET_PDOS_COMMAND_t PtrGetPdos)
{
  ...
  {
    maxnbPDOSNK = DPM_Ports[PortNumber].DPM_NumberOfRcvSNKPDO;
    maxnbPDOSRC = DPM_Ports[PortNumber].DPM_NumberOfRcvSRCPDO;
...
static USBPD_StatusTypeDef  PPM_GetConnectorStatus(uint8_t PortNumber)
{
  ...
  /* Check if device is connected or not */
  p_connector_status->ConnectStatus         = DPM_Params[PortNumber].PE_IsConnected;

  /* Fields valid only in connect status set to 1 */
  if (p_connector_status->ConnectStatus)
  {
    ...
    p_connector_status->ConnectorPartnerFlags = UcsiConnectorPartnerFlagUsb;
  ...
  if (p_connector_status->ConnectStatus && (UcsiPowerOperationModePd == p_connector_status->PowerOperationMode))
  {
                                                       ...
                                                       PUCSI_GET_CONNECTOR_CAPABILITY_IN_t PtrConnectorCapability)
{
  ...
  if (PORT0_PDO_ListSRC[0U] & USBPD_PDO_SRC_FIXED_USBCOMM_SUPPORTED)
  {
    PtrConnectorCapability->OperationMode.Usb2                = 0U;
  }
  PtrConnectorCapability->OperationMode.Usb3                = 0U;
  PtrConnectorCapability->OperationMode.AlternateMode       = DPM_Settings[PortNumber].PE_VDMSupport;
  ...
  PtrConnectorCapability->SwapToDFP = DPM_USER_Settings[PortNumber].PE_DR_Swap_To_DFP;
  PtrConnectorCapability->SwapToUFP = DPM_USER_Settings[PortNumber].PE_DR_Swap_To_UFP;
